# Product Catalog - Full-Stack Developer Test

A modern, high-performance product catalog application built with Next.js 14, featuring optimized rendering strategies (SSR, SSG, ISR) and dynamic search/filter capabilities. This project demonstrates best practices for building performant, SEO-friendly e-commerce applications.

This project implements a full-featured product catalog using FakeStoreAPI, with careful consideration given to rendering strategies, performance optimization, and SEO requirements. The application showcases hybrid rendering approaches that balance specification compliance with optimal user experience.

Built with Next.js 14 (App Router), TypeScript, and Tailwind CSS, this project demonstrates professional-grade code organization, error handling, and responsive design principles.

---

## Features

- **Home Page**: Fixed header displaying "HELLO WORLD"
- **Products Catalog**: Browse products with search and category filtering
- **Product Detail Pages**: View detailed product information including image, name, description, price, and category
- **Search & Filter**: Search products by name and filter by category with dynamic updates
- **Error Handling & Loading States**: Error boundaries for API failures and loading indicators during data fetching
- **404 Pages**: Custom 404 pages for invalid URLs and product IDs
- **Responsive Design**: Fully responsive layout optimized for all device sizes
- **SEO Optimization**: Server-rendered HTML, dynamic metadata, and optimized page structure for search engines

---

## Technology Stack

- **Next.js 14** (App Router) - React framework with Server Components and optimized rendering
- **TypeScript** - Type-safe development
- **Tailwind CSS** - Utility-first CSS framework for styling
- **FakeStoreAPI** - REST API for product data
...
4. **Access the application**:
   ...
   - The home page displays "HELLO WORLD"
   - Navigate to `/products` to see the product catalog

### Available Scripts

- `npm run dev` or `yarn dev` - Start development server (runs on port 3000)
- `npm run build` or `yarn build` - Build the application for production
- `npm start` or `yarn start` - Start the production server (requires build first)
- `npm run lint` or `yarn lint` - Run ESLint to check code quality

The project is ready to run locally after completing these steps.
...
│   ├── page.tsx                  # Home page displaying "HELLO WORLD"
│   ├── globals.css               # Global styles and Tailwind imports
│   ├── error.tsx                 # Global error boundary component
│   ├── not-found.tsx             # Global 404 page component
│   └── products/                 # Products routing section
│       ├── page.tsx              # Products catalog page (SSR)
│       ├── loading.tsx           # Loading UI for products page
│       ├── error.tsx             # Error boundary for products section
│       └── [id]/                 # Dynamic route for product details
│           ├── page.tsx          # Product detail page (SSG + ISR)
│           ├── loading.tsx       # Loading UI for product detail
│           ├── error.tsx         # Error boundary for product detail
│           └── not-found.tsx     # 404 page for invalid product IDs
│
├── components/                   # Shared React components
│   ├── BackButton.tsx            # Navigation back button component
│   ├── CategoryFilter.tsx        # Category filter dropdown (Client Component)
│   ├── Header.tsx                # Site header with "HELLO WORLD"
│   ├── LoadingSpinner.tsx        # Loading spinner component
│   ├── ProductCard.tsx           # Product card display component
│   ├── ProductList.tsx           # Product list with search (Client Component)
│   ├── ScrollRestoration.tsx     # Scroll position restoration component
│   └── SearchBar.tsx             # Search input component (Client Component)
│
├── lib/                          # Utility functions and helpers
│   ├── api.ts                    # API client functions with caching strategies
...
This project is created as a technical test submission and is available as open source.
